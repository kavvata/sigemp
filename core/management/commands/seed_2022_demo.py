...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        parser.add_argument("--registros", type=int, required=False)

    def handle(self, *args, **options):
        N: int = options.get("registros", 100) or 100
        ...
        try:
            with transaction.atomic():
                ...
                marcas_modelos_models = [
                    MarcaModelo.objects.get_or_create(
                        marca=mm["marca"], modelo=mm["modelo"]
                    )[0]
                ...
                formas_selecao_models = [
                    FormaSelecao.objects.get_or_create(**e.to_dict())[0]
                    for e in lista_entities
                ]
                ...
                for i in range(N):
                    e = BemEntity(
                        ...
                        grau_fragilidade_id=random.choice(
                            GrauFragilidade.objects.all()
                        ).id,
                        estado_conservacao_id=random.choice(
                            EstadoConservacao.objects.all()
                        ).id,
                ...
                for i in range(N):
                    alunos_disponiveis = Aluno.objects.exclude(
                        emprestimo__estado=1
                    ).distinct()

                    bens_disponiveis = Bem.objects.exclude(
                        emprestimo__estado=1
                    ).distinct()

                    if not alunos_disponiveis.exists() or not bens_disponiveis.exists():
                        print(
                            "Not enough available alunos or bens to create more empréstimos"
                        )
                        break
                    ...
                    entity = EmprestimoEntity(
                        ...
                        data_devolucao_prevista=datetime.now().date()
                        + timedelta(days=365),

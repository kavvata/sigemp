...
def gerar_post_data(aluno: AlunoEntity, exclude=["timestamps"]):
    if "timestamps" not in exclude:
        exclude.append("timestamps")

    aluno_dict = aluno.to_dict(exclude)
    for key, value in aluno_dict.items():
        if value is None:
            aluno_dict[key] = ""

    aluno_dict["curso"] = aluno_dict.pop("curso_id")
    aluno_dict["forma_selecao"] = aluno_dict.pop("forma_selecao_id")
...
def test_listar_alunos(alunos, admin_client):
    response = admin_client.get(reverse_lazy("ensino:listar_alunos"))

    for aluno in [aluno.nome for aluno in alunos]:
...
def test_criar_aluno(admin_client, aluno_entity: AlunoEntity):
    ...
    data = gerar_post_data(aluno_entity, ["id"])
...
@pytest.mark.django_db
def test_editar_aluno(admin_client, aluno, aluno_entity: AlunoEntity):
    ...
    update_data = gerar_post_data(aluno_entity)
...
def test_nao_pode_editar_aluno_matricula_repetida(
    admin_client, aluno: Aluno, lista_alunos: list[AlunoEntity]
):
    url = reverse_lazy("ensino:editar_aluno", args=[aluno.pk])
    ...
    data = gerar_post_data(aluno_entity)
...
def test_remover_aluno(admin_client, aluno):
    ...
    response = admin_client.get(reverse_lazy("ensino:listar_alunos"))
    assertNotContains(response, aluno.descricao)
